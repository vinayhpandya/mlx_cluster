...
kernel void random_walk(
    ...
    const device int32_t* col [[buffer(1)]],
    const device int32_t* start [[buffer(2)]],
    const device float* rand [[buffer(3)]],
    device int32_t* n_out [[buffer(4)]],
    device int32_t* e_out [[buffer(5)]],
    constant int& walk_length [[buffer(6)]],
    uint tid [[thread_position_in_grid]]
) {
    int32_t n_cur = start[tid];
    n_out[tid * (walk_length + 1)] = n_cur;
    
    for (int l = 0; l < walk_length; l++) {
        int32_t row_start = rowptr[n_cur];
        int32_t row_end = rowptr[n_cur + 1];
        int32_t e_cur;
